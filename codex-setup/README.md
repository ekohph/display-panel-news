...
- 첫 패스는 최신 public summary, `seen_articles.json`, 그리고 핵심 recurring source 목록만 읽고 종료합니다.
- 후보가 없는 경우 즉시 종료해 추가 탐색을 피합니다.
- broader web search는 후보가 발견된 뒤에만 수행합니다.
- 결과는 섹션당 1~3개 항목으로 압축하고, `원문:` 한 개만 기본으로 쓰며 `교차 확인:`은 필요할 때만 추가합니다.
